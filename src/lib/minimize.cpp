...
public:
    ...
    Minimizer(Automata& input) : input(input), marked(input.size(), std::vector<bool>(input.size())) {}
...
void minimize_cdfa(Automata& cdfa) {
    Minimizer minimizer(cdfa);
    minimizer.build_table();
    minimizer.used = cdfa.get_reachable_nodes();
    minimizer.minimize();
    cdfa = minimizer.output;
}
